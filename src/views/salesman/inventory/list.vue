    ...
    <div class="main kd-flex" v-loading="loading">
        ...
        <div class="container kd-flex-item kd-flex">
            <div>
                ...
                <p v-if="$util.checkPermission('item',['c'])" class="action">
            ...
            <div class="action">
                <el-radio-group size="mini" v-model="query.spuId">
                    ...
                    <el-radio-button v-for="(item,idx) in spuList" :key="idx" :label="item.id">{{item.spuName}}
                    </el-radio-button>
                </el-radio-group>
            </div>
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button type="text" @click="onEdit(scope.row)"
                                   v-if="$util.checkPermission('item',['d'])">编辑
                        </el-button>
                        <el-button type="text" @click="onDelete(scope.row)"
                                   v-if="$util.checkPermission('item',['d'])">删除
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
                ...
                <el-form :model="form" ref="form" class="inventory-form" :rules="rules">
                    ...
                    <el-form-item label="规格名称" prop="spuId" label-width="80px">
                        <el-select :disabled="!add" v-model="form.spuId" placeholder="请选择规格">
                            <el-option v-for="item in spuList" :key="item.value" :label="item.spuName"
